...
transformed parameters {
  real <lower = 0, upper = 1> true_seropositive[N];               // proportion truly dengue seropositive for each age-year group
  real <lower = 0, upper = 1> test_seropositive[N];               // proportion testing seropositive for each age-year group pre-zika
  ...
  for(i in 1:N) {
    true_seropositive[i] = 1 - exp(-sum(to_vector(modelled_lambda[X[i,1]:X[i,2]])));
